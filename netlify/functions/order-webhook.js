...
// Minimal version: ส่ง Flex เข้า LINE OA (ไม่ใช้ firebase-admin)

function formatTHB(n){ return new Intl.NumberFormat("th-TH",{style:"currency",currency:"THB"}).format(Number(n)||0); }
...
function buildOrderFlex(orderId, order){
  ...
  const total = Number(order.total) || items.reduce((s,it)=>s+(Number(it.price||0)*Number(it.qty||0)),0);
...
async function pushLineFlex({to, orderId, order}){
  const payload = {
    to,
    messages: [{ type:"flex", altText:`Order ${orderId} | ${order.channel||"-"}`, contents: buildOrderFlex(orderId, order) }]
  };
